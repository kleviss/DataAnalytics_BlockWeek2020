...
ggplot(totalNEI, aes(x = year, y = pollution)) + 
     ggtitle(expression("Total US "~ PM[2.5]~ "Emissions by Year")) +
      xlab("Year") +
...
cityB <- NEI  %>% 
    ...
    group_by(year) %>% 
    summarise(pollution = sum(Emissions))
ggplot(cityB, aes(x = year, y = pollution)) + geom_bar(stat = "identity") +
...
cityB <- NEI %>% 
    ...
    group_by(type, year) %>% 
    summarise(pollution = sum(Emissions))
ggplot(cityB, aes(fill=type, x = year, y = pollution)) + 
...
coalNEI <- NEI  %>% 
    ...
    group_by(year) %>% 
    summarise(pollution = sum(Emissions))
ggplot(coalNEI, aes(x = year, y = pollution)) + geom_bar(stat = "identity") +
...
data <- NEI  %>% 
    filter(SCC %in% eISecFilter$SCC & fips == "24510") %>% 
    group_by(year) %>% 
    summarise(pollution = sum(Emissions))
...
eISecFilter <- SCC %>% 
    filter(substr(EI.Sector, 1, 6) == "Mobile")
bandLA <- NEI  %>% 
    filter(SCC %in% eISecFilter$SCC & (fips == "24510" | fips == "06037")) %>% 
    group_by(fips,year) %>% 
    summarise(pollution = sum(Emissions)) %>%
    mutate( city = case_when(fips=="24510"~ "Baltimore City", fips == "06037" ~ "Los Angeles County"))
ggplot(bandLA, aes(fill=city, x = year, y = pollution)) + 
    geom_bar(position = position_dodge(),stat = "identity") +
