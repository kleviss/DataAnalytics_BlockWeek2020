...
```{r}
pollutant("./specdata/", "sulfate", 55)

## [1] 3.587319
```

```{r}
pollutant("./specdata/", "nitrate")
...
corr <- function(directory, threshold = 0) {
      ...
      # build a large data.table with the data from files
      fileList <- lapply(file.path(directory, list.files(path = directory, pattern="*.csv")), data.table::fread)
      data <- rbindlist(fileList)
